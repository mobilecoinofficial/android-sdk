...
public class SenderMemoTest {
  ...
  // This data was generated from the rust code. tl;dr- it's the data that's used to
  ...
  private final String receivingSubaddressViewPrivateKeyHexProtoBytes = "0a203a160e2f5931213b090ad73e06cb5c90067a196ee177d4233e16982f49ea6007";
  ...
  public void getSenderMemoData_validMemoData_returnsSenderMemoDataWithCorrectAddressHash() throws Exception {
    ...
    SenderMemo senderMemo = SenderMemo.create(memoData);

    PublicAddress senderPublicAddress =
        PublicAddress.fromBytes(Hex.toByteArray(senderPublicAddressHexProtoBytes));
    MobileCoinAPI.RistrettoPrivate privateKeyProto =
        MobileCoinAPI.RistrettoPrivate
            .parseFrom(Hex.toByteArray(receivingSubaddressViewPrivateKeyHexProtoBytes));
    RistrettoPrivate receivingSubaddressViewPrivateKey =
        RistrettoPrivate.fromBytes(privateKeyProto.getData().toByteArray());
    MobileCoinAPI.CompressedRistretto compressedRistrettoProto =
        MobileCoinAPI.CompressedRistretto.parseFrom(Hex.toByteArray(txOutPublicKeyHexProtoBytes));
    RistrettoPublic txOutPublicKey =
        RistrettoPublic.fromBytes(compressedRistrettoProto.getData().toByteArray());

    SenderMemoData senderMemoData = senderMemo.getSenderMemoData(
        senderPublicAddress,
  ...
  public void getUnvalidatedAddressHash_returnsAddressHash() {
   SenderMemo senderMemo = SenderMemo.create(new byte[44]);
